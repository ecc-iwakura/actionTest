	...
	for (Material& material : materials)
	{
		...
		if (FAILED(hr))
		{
			...
			else
			{
				...
				const int dummyWidth = 8;
				const int dummyHeight = 8;
				UINT dummyPixels[dummyWidth * dummyHeight];
				::memset(dummyPixels, 0xFF, sizeof(dummyPixels));

				D3D11_TEXTURE2D_DESC desc = { 0 };
				desc.Width = dummyWidth;
				desc.Height = dummyHeight;
				desc.MipLevels = 1;
				desc.ArraySize = 1;
				desc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
				desc.SampleDesc.Count = 1;
				...
				D3D11_SUBRESOURCE_DATA data;
				::memset(&data, 0, sizeof(data));
				data.pSysMem = dummyPixels;
				data.SysMemPitch = dummyWidth;
